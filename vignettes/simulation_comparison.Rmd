---
title: "Inference Comparison: OLS, HC3, Wild Bootstrap, Permutation"
output:
  rmarkdown::html_vignette:
    toc: true
vignette: >
  %\VignetteIndexEntry{Inference Comparison: OLS, HC3, Wild Bootstrap, Permutation}
...
knitr::opts_chunk$set(echo = TRUE, message = FALSE, warning = FALSE,
                      cache = FALSE, fig.width = 7, fig.height = 4.5)
```

## Overview

This vignette evaluates four inference procedures available in `lwdidR` via a
Monte Carlo simulation study. The methods are:
...
The wild bootstrap and permutation tests are **distribution-free** alternatives
that are expected to maintain correct size at small N even when errors are
non-normal, at the cost of additional computation.

## Data-Generating Process

We consider a balanced common-timing panel:

- $N$ units (half treated), $T_{\text{pre}} = 3$, $T_{\text{post}} = 3$
- $y_{it} = \alpha_i + \beta D_{it} + \varepsilon_{it}$
- $\alpha_i \sim N(0, 1)$ (removed by demeaning)
- $\varepsilon_{it} \sim N(0,1)$ (**normal**) or $\varepsilon_{it} \sim t(3)$
  (**heavy tails**)
- $\beta = 0$ for **size** experiments; $\beta = 0.2$ for **power / coverage**
...
$N \in \{20, 50, 100\}$, error distribution, treatment status ($\beta = 0$ or
$0.2$), and the four inference methods. Bootstrap and permutation replications
are set to 199 for speed.

```{r run-simulation, cache=FALSE}
set.seed(42)
S       <- 500L
...
```{r se-boxplot, fig.cap="SE distribution across simulations (β = 0.2, normal errors). Boxes summarise 500 replications. Wildboot and permutation SEs reflect the variability of randomisation-based estimates."}
...
```{r se-boxplot-t3, fig.cap="SE distribution under heavy-tailed (t(3)) errors."}
...
1. **Size control at small N.** With $N = 20$, the homoskedastic OLS t-test
   and HC3 can over-reject when errors are heavy-tailed. Wild bootstrap and
   permutation inference maintain empirical size closer to the nominal 5%
   level by using the finite-sample randomisation distribution rather than
   asymptotic critical values.

2. **Power.** All methods achieve similar power at $N = 100$. At small N, the
   distribution-free methods may sacrifice modest power in exchange for
   correct size.

3. **Coverage.** Coverage tracks the size results: methods that over-reject
   under $H_0$ will exhibit under-coverage under $H_1$.

**Recommendation.** Use `vce = "hc3"` as the default. When $N \lesssim 30$ or
errors are suspected to be non-normal, prefer `vce = "wildboot"` or
`vce = "permutation"` with at least `nboot = 499` replications.
